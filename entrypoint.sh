...
echo "\n===Expenses Last Month==\n" >> result.txt
ledger -p "last month" --cleared-format="%(justify(scrub(get_at(display_total, 0)), 16, 16 + int(prepend_width),true, color))  %-(ansify_if(partial_account(options.flat), blue if color))\n" cleared Expenses:日元 -f $FILE_PATH >> result.txt
